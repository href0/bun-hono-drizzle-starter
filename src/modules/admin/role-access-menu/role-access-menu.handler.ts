...
// import { roleAccessMenuRoute } from './role-access-menu.route';
// const menuHandler = new OpenAPIHono({ defaultHook: defaultHookConfig() })

// menuHandler.openapi(roleAccessMenuRoute.create, async (c) => {
...
// menuHandler.openapi(roleAccessMenuRoute.findAll, async (c) => {
...
// menuHandler.openapi(roleAccessMenuRoute.findOne, async (c) => {
//   const { id } = c.req.valid('param')
//   const user = await menuService.findOne(id)
//   return responseJson.OK(c, user, 'Menu found')
// })

// menuHandler.openapi(roleAccessMenuRoute.update, async (c) => {
//   const body = c.req.valid('json')
...
// menuHandler.openapi(roleAccessMenuRoute.remove, async (c) => {
//   const id = c.req.valid('param').id
